...
@endforeach
    ...
    <form method="post" action="{{ route('login') }}">
        ...
        <div class="form-group mb-4">
            ...
            <input type="text" name="login" class="form-control rounded-pill" value="{{ old('login') }}">
            @error('login')
        ...
        @if (Route::has('password.request'))
            <p class="text-center"><a class="link-primary" href="{{ route('password.request') }}">Mot de passe oublié ?</a></p>
        @endif
    </form>
</div>

<div class="card-footer bg-white text-center pt-0">
    <p class="mb-0">Vous n'avez pas de compte ? <a href="{{ route('register') }}" class="link-primary">S'enregistrer</a></p>
</div>
</div>
</div>
</div>
